//shortDescription length should not be greater than 104 characters including spaces and special characters.

export const members = [
  {
    id: 1,
    name: 'Aditya Singh',
    batch: '2024-28',
    shortDescription: "Full-stack developer building scalable web apps and strengthening DSA through consistent problem solving",
    linkedIn: "https://www.linkedin.com/in/aditya-singh-82a341320/",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/aditya.jpg'
  },
  {
    name: 'Aditi Pandey',
    batch: '2024-28',
    shortDescription: "Focused on learning Data Structures and improving problem-solving skills",
    linkedIn: "https://www.linkedin.com/in/aditi-pandey-689800274",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/aditi.jpg'
  },
  {
    id: 3,
    name: "Bipin Pal",
    batch: '2024-28',
    shortDescription: "Computer Science student passionate about AI, Machine Learning, Robotics, and Full-Stack Development.....",

    linkedIn: "https://www.linkedin.com/in/bipinpal12",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/bipin.jpg'
  },
  {
    id: 6,
    name: 'Shagun',
    batch: '2024-28',
    shortDescription: "Exploring full-stack, embedded systems, and Web3 through projects and open-source",
    linkedIn: "https://www.linkedin.com/in/shagunn06",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/shagun.jpg'
  },
  {
    id: 2,
    name: 'Raj Shekhar Patel',
    batch: '2024-28',
    shortDescription: "Focused on building clean, responsive, and user-friendly web interfaces",
    linkedIn: "https://www.linkedin.com/in/raj-shekhar-patel-332782329/",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/raj.jpg'
  },

  {
    name: "Priyanshu Jaiswal",
    batch: '2024-28',
    shortDescription: "Data Science and ML undergrad",
    linkedIn: "https://www.linkedin.com/in/priyanshujaiswal06",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/priyanshu.jpg'
  },
  {
    name: "Sachin Pandey",
    batch: '2023 - 27',
    shortDescription: "IT Undergrad Student, Data analyst, transforming numbers into strategic decisions.",
    linkedIn: "https://www.linkedin.com/in/sachin-pandey-954467289",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/sachin.jpg'
  },
  {
    name: "Kartikey Vishwakarma",
    shortDescription: "Research Intern @ IIIT Naya Raipur | MERN developer building government portals with Google Cloud",
    linkedIn: "https://www.linkedin.com/in/kartikey28",
    batch: '2023-27',
    bg: '/images/cardBg.png',
    profileImg: '/images/members/kartikey.jpg'
  },
  {
    name: "Harikesh",
    shortDescription: "AI/ML engineer with internship experience in data analytics, building insights from large datasets.",
    linkedIn: "https://www.linkedin.com/in/harikesh-prajapati-05034027a",
    batch: '2023-27',
    bg: '/images/cardBg.png',
    profileImg: '/images/members/harikesh.jpg'
  },

  {
    name: "Sarvesh Pandey",
    shortDescription: "Cse Undergrad IET Rmlau 2023-27",
    linkedIn: "https://www.linkedin.com/in/sarveshpandey01/",
    batch: '2023-27',
    bg: '/images/cardBg.png',
    profileImg: '/images/members/sarvesh.jpg'
  },


  {
    name: "Shivam Verma",
    batch: '2023-27',
    shortDescription: "Passionate about MERN Stack and React Native, building innovative solutions for real-world challenges.",
    linkedIn: "https://www.linkedin.com/in/shivam-verma-91327a293",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/shivam.jpg'
  },
  {
    name: "Aditya Mishra",
    batch: '2022-26',
    shortDescription: "Passionate about Full-Stack and Generative AI, building scalable applications for real-world problems.",

    linkedIn: "https://www.linkedin.com/in/aditya-mishra-6b587322b",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/adityaMishra.jpg'
  },
  {
    "name": "Anurag Pasi",
    batch: '2022-26',
    "shortDescription": "Software engineer building AI-powered ERP, ML solutions, and business automation with Python and React.",

    linkedIn: "https://www.linkedin.com/in/anurag-pasi-80a138259/",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/anurag.jpg'
  },
  {
    name: "Rajat Pandey",
    batch: '2022-26',
    shortDescription: "GATE 2026, Full stack developer, Constructing ERP models .",
    linkedIn: "https://www.linkedin.com/in/rajat-pandey-661a29248",
    bg: '/images/cardBg.png',
    profileImg: '/images/members/rajat.jpg'
  },

]